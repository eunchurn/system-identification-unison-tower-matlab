...
% Simple Stochastic Subspace Identification Estimator
% INPUT
%   data : mxn DATA Matrics n=DOFs, m=data length
%   Fs : sampling rate (Hz)
%   freqband : Frequency band of interest. It make output with analyzed Eigenvalues
%   minorder : Minimum order of singular value decompositions
%   maxorder : Maximum order of singular value decompositions
% OUTPUT
%   na_freq : output of ERA frequency
%   modevec : output of ERA Eigenvector
%   xi : damping ratio of ERA
%   na_freq_cov : Covariance of ERA Eigenvalues
%   modevec_cov : Covariance of ERA Eigenvectors
%   xi_cov : Covariance of ERA damping ratio
%   estVarBRfreq : Estimated frequency variable via SSI result
%   estVarBRzera : Estimated zeta variable vis SSI result
...
%%
disp(['Build Modal Stability Chart'])
tempVar=[];
...
critical_frequency_difference=0.01;
critical_zeta_ratio=0.50;
critical_damping=0.20;
critical_MAC_value=0.95;
...
m=[];
    for Norder=minorder:maxorder
        for i=1:length(estVarBR{Norder}.freq)
            if (estVarBR{Norder}.stability(i)==2 & estVarBR{Norder}.freq(i)>freqband(kk,1) & estVarBR{Norder}.freq(i)<freqband(kk,2))
